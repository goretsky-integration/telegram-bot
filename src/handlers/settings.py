...
async def on_switch_all_unit_statuses_button(
    ...
    match action:
        case 'enable':
            await db.add_unit_ids_to_report(report_type, chat_id, unit_ids_by_region)
        case 'disable':
            await db.remove_unit_ids_from_report(report_type, chat_id, unit_ids_by_region)

    enabled_reports = await db.get_reports(report_type=report_type, chat_id=chat_id)
...
async def on_switch_unit_statis_button(
    ...
    if is_unit_enabled:
        await db.remove_unit_ids_from_report(report_type, callback_query.message.chat.id, [unit_id])
    else:
        await db.add_unit_ids_to_report(report_type, callback_query.message.chat.id, [unit_id])
    units_by_region = await db.get_units(region)
    unit_ids_by_region = {unit.id for unit in units_by_region}
    enabled_reports = await db.get_reports(report_type=report_type, chat_id=callback_query.message.chat.id)
    enabled_unit_ids = [unit_id for report in enabled_reports for unit_id in report.unit_ids]
...
async def on_region_units_button(
    ...
    enabled_units = await db.get_reports(report_type=report_type, chat_id=callback_query.message.chat.id)
