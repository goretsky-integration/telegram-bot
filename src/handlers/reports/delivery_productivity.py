...
from services.converters import UnitsConverter, to_delivery_productivity_statistics_view_dto
...
async def on_delivery_productivity_statistics_report(
    ...
    logging.info('Delivery productivity statistics report request')
    ...
    reports = await get_v2_statistics_reports_batch(
        api_method=dodo_api_service.get_delivery_productivity_statistics_report,
        units_grouped_by_account_name=units.grouped_by_account_name,
        accounts_tokens=accounts_tokens,
    )
    units_delivery_speed_statistics = to_delivery_productivity_statistics_view_dto(reports, units.unit_uuid_to_name)
    view = DeliveryProductivityStatisticsView(units_delivery_speed_statistics)
    await answer_views(report_message, view, edit=True)
    logging.info('Delivery productivity statistics report sent')
